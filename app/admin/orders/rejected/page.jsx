...
export default function RejectedOrders() {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const response = await api.get('/rejected-orders', {
        params: {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 p-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h2 className="text-xl sm:text-2xl font-bold">Rejected Orders</h2>
      </div>
            ...
            {isLoading ? (
              ...
              orders.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell className="truncate max-w-[200px]">{item.rejectionReason || 'N/A'}</TableCell>
                  <TableCell>
                    <DropdownMenu>
