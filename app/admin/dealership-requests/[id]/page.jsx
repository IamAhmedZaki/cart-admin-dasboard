...
export default function DealerRegistrationDetails() {
  ...
  useEffect(() => {
    const fetchDealer = async () => {
      ...
      try {
        ...
        if (data.interestedBrands && typeof data.interestedBrands === "string") {
        ...
        if (data.authorizedDealer && typeof data.authorizedDealer === "string") {
  ...
  if (!dealer) return <div className="text-center py-8">Dealer registration not found</div>;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="md:col-span-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-2">
              ...
              <Input value={`${dealer.billingCity}, ${dealer.billingState} ${dealer.billingZip}`} disabled />
              <Input value={dealer.billingCountry} disabled className="sm:col-span-2" />
            </div>
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-2">
              ...
              <Input value={`${dealer.commercialCity}, ${dealer.commercialState} ${dealer.commercialZip}`} disabled />
              <Input value={dealer.commercialCountry} disabled className="sm:col-span-2" />
            </div>
          </div>

          <div>
            <Label>Has Showroom?</Label>
            <Input value={dealer.hasShowroom === "yes" ? "Yes" : "No"} disabled />
          </div>
      ...
      <SelectItem value="approved">Approved</SelectItem>
          ...
          {dealer.interestedBrands && (
            <div className="md:col-span-2">
              ...
              <Input value={Array.isArray(dealer.interestedBrands) ? dealer.interestedBrands.join(", ") : "None"} disabled />
            </div>
          )}
          {dealer.sellBrands && (
            <div className="md:col-span-2">
              <Label>Currently Sells Brands</Label>
              <Input value={Array.isArray(dealer.sellBrands) ? dealer.sellBrands.join(", ") : "None"} disabled />
            </div>
          )}
          {dealer.authorizedDealer && (
            <div className="md:col-span-2">
              <Label>Authorized Dealer For</Label>
              <Input value={Array.isArray(dealer.authorizedDealer) ? dealer.authorizedDealer.join(", ") : "None"} disabled />
            </div>
          )}

          <div>
            <Label>Submitted On</Label>
            <Input value={new Date(dealer.createdAt).toLocaleDateString()} disabled />
          </div>
        </div>
